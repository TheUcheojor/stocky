...
class LongShort {
  constructor(
    email,
    { apiKey, secretKey, stocks, paper = true, bucketPct = 0.25 }
  ) {
  ...
  getPercentChanges(limit = 10) {
    return Promise.all(
      this.stockList.map((stock) => {
        return new Promise(async (resolve) => {
          try {
            let resp = await this.alpaca.getBars("minute", stock.name, {
              limit: limit,
            });
            // polygon and alpaca have different responses to keep backwards
            // compatibility, so we handle it a bit differently
...
LongShort.remove = function (user) {
  LongShort.instances = LongShort.instances.filter((longShortInstance) => {
    ...
    if (
      ...
      console.log(
        `\nRemoving currently-running strategy (${longShortInstance.getStrategyName()}) of ${
          user.email
        }`
      );
