...
import strategyLog from "../strategyLog.js";
...
class MeanReversion {
  ...
  async runStategyOnStock(stock) {
    ...
    var promBars = new Promise((resolve, reject) => {
      var barChecker = setInterval(async () => {
        await this.alpaca.getCalendar(Date.now()).then(async (resp) => {
          ...
          await this.alpaca
            .getBars("minute", stock, { start: marketOpen })
            .then((resp) => {
              var bars = resp[stock];
              if (bars.length >= 20) {
    ...
    var spin = setInterval(async () => {
      ...
      if (this.lastOrders[stock] != null)
        await this.alpaca
          .cancelOrder(this.lastOrders[stock].id)
          .catch((err) => {
            this.log(err.error);
          });
  ...
  async rebalance(stock) {
    ...
    if (currPrice > this.runningAverages[stock]) {
      ...
      if (positionQuantity > 0) {
        ...
        this.log(
          "No position in the " + stock + " stock.  No action required."
        );
      ...
      var portfolioShare =
        ((this.runningAverages[stock] - currPrice) / currPrice) * 200;
...
MeanReversion.remove = function (user) {
  MeanReversion.instances = MeanReversion.instances.filter(
    (meanReversionInstance) => {
      const instanceAuthentication = meanReversionInstance.getAuthentication();
      const userAuthentication = user.alpaca;

      if (
        instanceAuthentication.apiKey == userAuthentication.apiKey &&
        instanceAuthentication.secretKey == userAuthentication.secretKey
      ) {
        console.log(
          `\nRemoving currently-running strategy (${meanReversionInstance.getStrategyName()}) of ${
            user.email
          }`
        );
