...
import Strategy from "../trading/strategyController.js";
import connectToAlpaca from "../auxilliary/connectToAlpaca.js";
import STRATEGIES from "../trading/strategyReference.js";
...
settingsRouter.post("/set-password", (req, res) => {
  ...
  User.updateOne(
    ...
    .catch((error) => {
      ...
      res.status(500).json({
        success: false,
        message: `Cannot update password associated to email: ${email}`,
        error: `${error}`,
      });
    });
});
...
settingsRouter.get("/get-strategies", (req, res) => {
  ...
  User.findOne({ email: email })
    ...
    .catch((error) => {
      ...
      res.status(500).json({
        success: false,
        message: `Cannout find user ${email}`,
        error: `${error}`,
      });
    });
});
...
settingsRouter.post("/set-strategy", (req, res) => {
  ...
  User.updateOne(
    ...
    .then(() => {
      User.findOne({ email: email })
        ...
        .catch((error) => {
          ...
          res.status(500).json({
            success: false,
            message: `Cannot find account associated to email: ${email}`,
            error: `${error}`,
          });
        });
    })
    .catch((error) => {
      ...
      res.status(500).json({
        success: false,
        message: `Cannot set strategy: ${strategy}`,
        error: `${error}`,
      });
    });
});
...
settingsRouter.post("/add-stock", (req, res) => {
  ...
  User.updateOne(
    { email: email },
    { $addToSet: { "alpaca.stocks": stockSymbol } }
  )
    .then(() => {
      User.findOne({ email: email })
        .then((user) => {
    ...
    .catch((error) =>
      res.status(500).json({
        success: false,
        message: `Cannot add stock: ${stock}`,
        error: `${error}`,
      })
    );
});
...
settingsRouter.delete("/remove-stock", (req, res) => {
  ...
  User.updateOne({ email: email }, { $pull: { "alpaca.stocks": stockSymbol } })
    ...
    .catch((error) => {
      ...
      res.status(500).json({
        success: false,
        message: `Cannot remove the following stock: ${stockSymbol}`,
        error: `${error}`,
      });
    });
});
