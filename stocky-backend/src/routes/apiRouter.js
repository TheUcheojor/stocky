...
import connectToAlpaca from "../auxilliary/connectToAlpaca.js";
  ...
  User.findOne({ email: email })
    .then((user) => {
      ...
      alpaca
        ...
        .then((activities) => {
          ...
          async
            .each(activities, function (activity, callback) {
              alpaca
                .getOrder(activity.order_id)
                .then((order) => {
            ...
            .then(() => {
              ...
              orderedIds.forEach((orderId) => {
                let targetActivity = formattedActivities.find(
                  (activity) => activity.order_id == orderId
                );
            ...
            .catch((err) => {
              ...
              res
                ...
                .json({
                  success: false,
                  message: `Error getting account activities`,
                  error: `${err}`,
                });
            });
        })
        .catch((error) => {
          ...
          res
            ...
            .json({
              success: false,
              message: `Cannot connect to Alpaca`,
              error: `${error}`,
            });
        });
    })
    .catch((error) => {
      ...
      res
        ...
        .json({
          success: false,
          message: "No User associated with the given email",
          error: `${error}`,
        });
    });
});
...
apiRouter.get("/search", (req, res) => {
  ...
  const STOCK_PROFILE_API =
    "https://TheUcheojor.github.io/stocky-reference-data/stock-profiles.json";
  const SIMILARITY_CONSTANT = 0.3;

  console.log(
    `Requesting https://TheUcheojor.github.io/stocky-reference-data/stock-profiles.json for ${query}`
  );

  axios
    .get(STOCK_PROFILE_API)
    .then((axiosResponse) => {
      let stockProfiles = axiosResponse.data;

      let desiredStockProfiles = stockProfiles.filter((stockProfile) => {
        console.log(
          `${query} : StringSimilarity.compareTwoStrings(stockProfile.symbol, query ):  ${StringSimilarity.compareTwoStrings(
            stockProfile.symbol.toLowerCase(),
            query.toLowerCase()
          )}`
        );
        console.log(
          `${query} : StringSimilarity.compareTwoStrings(stockProfile.name, query ) : ${StringSimilarity.compareTwoStrings(
            stockProfile.name.toLowerCase(),
            query.toLowerCase()
          )}`
        );
    ...
    .catch((err) => {
      res
        ...
        .json({
          success: false,
          message: `Server Error. Cannot access stock profiles`,
          error: `${err}`,
        });
    });
});
...
apiRouter.get("/stocks", (req, res) => {
  ...
  console.log(
    `Requesting https://TheUcheojor.github.io/stocky-reference-data/stock-profiles.json for ${name}`
  );
  ...
  axios
    ...
    .then((axiosResponse) => {
      ...
      let desiredStockProfile = stockProfiles.find(
        (stockProfile) =>
        ...
        User.findOne({ email: email })
          .then((user) => {
            stockProfiles.forEach((stockProfile, i) => {
              stockProfiles[i].isInStockCollection =
                user.alpaca.stocks.includes(stockProfile.symbol);
            });
          ...
          .catch((error) => {
            ...
            res
              ...
              .json({
                success: false,
                message: `Server Error. Cannot access user profile`,
                error: `${error}`,
              });
          });
      } else {
      ...
      userAlpaca
        ...
        .catch((error) => {
          ...
          res
            ...
            .json({
              success: false,
              message: `Error creating ${type} ${side} order for ${symbol}`,
              error: `${error}`,
            });
        });
    })
      ...
      userAlpaca
        ...
        .catch((error) => {
          ...
          res
            ...
            .json({
              success: false,
              messeage: `Cannot get the portfolio history of ${email}`,
              error: `${error}`,
            });
        });
    })
  ...
  User.findOne({ email: email })
    ...
    .catch((error) => {
      ...
      res
        ...
        .json({
          success: false,
          message: `Cannout find user with email ${email}`,
          error: `${error}`,
        });
    });
});
